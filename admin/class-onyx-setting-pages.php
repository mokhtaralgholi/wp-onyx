  ...
  public function onyx_module_to_sync(){
	...
	public function api_settings(){
       ...
       array('key'=>'onyx_sms_message_parameter_name','title'=>'SMS Message Parameter Name'),
        ...
        array('key'=>'onyx_attribute_type','title'=>'Sync attribute option','type'=>'radio', "options"=>array("attachments"=>"Attachments","bathes"=>"Batches"))
		);
      ...
      global $wpdb;
         ...
         foreach($synclog['added'] as $line){
			...
			public function Sync_log_page(){
				...
				$onyxLogs = new Onyx_Admin_Sync_Logs( $this->plugin_name, $this->version);
				$currentlog = $onyxLogs->read_log_by_day($active_tab);
				?>
				<div class="wrap">
					<?php settings_errors(); ?>
					<h1> Sync Log</h1>
					<div class="description"><?php echo __("Sync Log for all operations over API");?></div>
					<h2 class="nav-tab-wrapper syncLogWrapper">
						<a href="?page=onyx_admin_sync_log&day=Sat" class="nav-tab <?php echo $active_tab == 'Sat' ? 'nav-tab-active' : ''; ?>">Sat</a>
						<a href="?page=onyx_admin_sync_log&day=Sun" class="nav-tab <?php echo $active_tab == 'Sun' ? 'nav-tab-active' : ''; ?>">Sun</a>
						<a href="?page=onyx_admin_sync_log&day=Mon" class="nav-tab <?php echo $active_tab == 'Mon' ? 'nav-tab-active' : ''; ?>">Mon</a>
						<a href="?page=onyx_admin_sync_log&day=Tue" class="nav-tab <?php echo $active_tab == 'Tue' ? 'nav-tab-active' : ''; ?>">Tue</a>
						<a href="?page=onyx_admin_sync_log&day=Wed" class="nav-tab <?php echo $active_tab == 'Wed' ? 'nav-tab-active' : ''; ?>">Wed</a>
						<a href="?page=onyx_admin_sync_log&day=Thu" class="nav-tab <?php echo $active_tab == 'Thu' ? 'nav-tab-active' : ''; ?>">Thu</a>
						<a href="?page=onyx_admin_sync_log&day=Fri" class="nav-tab <?php echo $active_tab == 'Fri' ? 'nav-tab-active' : ''; ?>">Fri</a>
					</h2>
